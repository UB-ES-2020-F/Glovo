...
trap ./clean_up.sh EXIT
trap ./clean_up.sh HUP
...
echo -e "${Yellow}Press Ctrl+C to stop server${Color_off}"
echo -e "${Yellow}Warning: If you close the terminal, the docker container will not be cleaned up automatically.${Color_off}"
read -r -d '' _</dev/tty
